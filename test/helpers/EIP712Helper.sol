...
abstract contract EIP712Helper is Test {
    ...
    string internal constant CHEQUE_TYPE =
    ...
    function signCheque(
        ...
        bytes32 structHash = vm.eip712HashStruct(
            CHEQUE_TYPE,
            abi.encode(chequebook, beneficiary, cumulativePayout)
        );
    ...
    function signCashout(
        ...
        bytes32 structHash = vm.eip712HashStruct(
            CASHOUT_TYPE,
            abi.encode(chequebook, sender, requestPayout, recipient, callerPayout)
        );
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", _domainSeparator(), structHash));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(privateKey, digest);
        return abi.encodePacked(r, s, v);
    ...
    function signCustomDecreaseTimeout(
        ...
        bytes32 structHash = vm.eip712HashStruct(
            CUSTOMDECREASETIMEOUT_TYPE,
            abi.encode(chequebook, beneficiary, decreaseTimeout)
        );
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", _domainSeparator(), structHash));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(privateKey, digest);
        return abi.encodePacked(r, s, v);
