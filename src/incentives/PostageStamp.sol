...
contract PostageStamp is OwnableRoles {
    using HitchensOrderStatisticsTreeLib for HitchensOrderStatisticsTreeLib.Tree;

    // ----------------------------- Role Constants ------------------------------

    // Role allowed to increase totalOutPayment.
    uint256 public constant PRICE_ORACLE_ROLE = 1 << 0;
    // Role allowed to pause
    uint256 public constant PAUSER_ROLE = 1 << 1;
    // Role allowed to withdraw the pot.
    uint256 public constant REDISTRIBUTOR_ROLE = 1 << 2;
    ...
    error ExpectedPause(); // Contract is not paused
    ...
    constructor(address _bzzToken, uint8 _minimumBucketDepth) {
        ...
        _grantRoles(msg.sender, PAUSER_ROLE);
        bzzToken = _bzzToken;
    ...
    function createBatch(
        ...
        if (!IERC20(bzzToken).transferFrom(msg.sender, address(this), totalAmount)) {
     ...
     */
    function setPrice(uint256 _price) external onlyRoles(PRICE_ORACLE_ROLE) {
        if (lastPrice != 0) {
    ...
    function setMinimumValidityBlocks(uint64 _value) external onlyOwner {
    ...
    function totalPot() public returns (uint256) {
        expireLimited(type(uint256).max);
        uint256 balance = IERC20(bzzToken).balanceOf(address(this));
    ...
    function withdraw(address beneficiary) external onlyRoles(REDISTRIBUTOR_ROLE) {
        uint256 totalAmount = totalPot();
        if (!IERC20(bzzToken).transfer(beneficiary, totalAmount)) {
            revert TransferFailed();
        }
     ...
     */
    function pause() public onlyRoles(PAUSER_ROLE) whenNotPaused {
        _paused = true;
     ...
     */
    function unPause() public onlyRoles(PAUSER_ROLE) whenPaused {
        _paused = false;
