...
contract StakeRegistry is OwnableRoles {
    // ----------------------------- Role Constants ------------------------------

    // Role allowed to freeze and slash entries
    uint256 public constant REDISTRIBUTOR_ROLE = 1 << 0;
    ...
    function withdrawFromStake() external {
        ...
        if (_surplusStake > 0) {
            ...
            if (!IERC20(bzzToken).transfer(msg.sender, _surplusStake)) revert TransferFailed();
    ...
    function migrateStake() external whenPaused {
        ...
        if (lastUpdatedBlockNumberOfAddress(msg.sender) != 0) {
            if (!IERC20(bzzToken).transfer(msg.sender, stakes[msg.sender].potentialStake)) revert TransferFailed();
     ...
     */
    function freezeDeposit(address _owner, uint256 _time) external onlyRoles(REDISTRIBUTOR_ROLE) {
     ...
     */
    function slashDeposit(address _owner, uint256 _amount) external onlyRoles(REDISTRIBUTOR_ROLE) {
        if (stakes[_owner].lastUpdatedBlockNumber != 0) {
    ...
    function changeNetworkId(uint64 _NetworkId) external onlyOwner {
        NetworkId = _NetworkId;
    }
    ...
    /**
     * @dev Unpause the contract, can only be called by the owner when paused
     */
